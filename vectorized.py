...
def multi_step(x:torch.Tensor):
    pointer_idx = 0
    step = 0
    while(x[pointer_idx] > x[x[pointer_idx]]):
        step = step + 1
        # print(f"pointer_idx: {pointer_idx}, step {step}")
        pointer_idx = x[pointer_idx]

    return x[pointer_idx], step
...
class VectorPVR(Dataset):
    ...
    def __init__(self,
        ...
        self.steps = torch.zeros(size, dtype=self.dtype)
        
        # get random value from 1 - 3, we only use "<" for multi-steps
        # self.steps = torch.randint(low=1,high =4 ,size = self.size)
        if aggregation_method != 'multi-step':
            for idx, sample in enumerate(self.data):
                # torch.sort(sample)
                self.values[idx] = self.calc_value(sample)
        else:
            for idx, sample in enumerate(self.data):
                # sample = torch.sort(sample, descending=True)[0]
                self.values[idx], self.steps[idx] = self.cal_multi_step(sample)
    ...
    def _set_up_aggregator(aggregation_method):
        if aggregation_method == 'mod_sum':
            ...
            aggregator = maj_vote
        elif aggregation_method == 'multi-step':
            aggregator = multi_step
        else:
    ...
    def cal_multi_step(self, sample):
        # pointer_idx = sample[0] + 1
...
def main():
    p = 2
    ds = VectorPVR(size=10**p, complexity=2, holdout=2, aggregation_method='multi-step', adversarial=False, name="test")
    print(p, ':', _get_tensor_size_mb(ds.data))
    import pdb
    pdb.set_trace()
    for idx in range(len(ds)):
        if ds.aggregation_method != 'multi-step':
            print(ds[idx])
        else:
            print(f"{ds.data[idx]}  {ds.values[idx]}  {ds.steps[idx]}")
