...
def map_to_coordinates(value):
    for key, values in HOLDOUT_CLASSES.items():
        if value in values:
            return key
    return None
...
class BlockStylePVR(Dataset):
    def __init__(self,
        ...
        self.steps = torch.zeros(self.pvr_ds_size, dtype=torch.long)
        self.value = torch.zeros(self.pvr_ds_size, dtype=torch.long)
        ...
        if mode == 'iid':
            ...
            print(f"Multi-step size: {self.pvr_ds_size}")
            for i, holdout_class in HOLDOUT_CLASSES.items():
                probs = torch.ones(len(self.ds))
                for label in holdout_class:
                    probs[ds_labels == label] = 0
            ...
            for idx in range(len(self.labels)):
                tmp = []
                for tmp_label in self.labels[idx]:
                    tmp.append(map_to_coordinates(tmp_label))
                # print(self.labels[idx])
                # print(tmp)
                step = 0
                pointer = 0 
                while(self.labels[idx][pointer] > self.labels[idx][map_to_coordinates(self.labels[idx][pointer])]):
                    if(idx == 0):
                        print(f"current value {self.labels[idx][pointer]}, current pointer {pointer}")
                        print(f"next value {self.labels[idx][map_to_coordinates(self.labels[idx][pointer])]}, next pointer {tmp[pointer]}")
                    step = step + 1
                    pointer = tmp[pointer]
                self.steps[idx] = step
                self.value[idx] = self.labels[idx][pointer]
        else:
...
def main():
    ...
    ds = BlockStylePVR(train=True, mode='multi-step', size=None)
    # print(len(ds))
    # for i in range(50):
    #     x, y = ds[i]
